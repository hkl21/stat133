...
name <- "Hyeonguk Lee"
github.name <- "hkl21"
email.address <- "hkl21@berkeley.edu"
...
set.seed(344)
m <- matrix(rnorm(10^2,mean=8),nrow=10,byrow=F)
...
cw3 <- mean(ChickWeight[ChickWeight$Time ==0,1])
...
# 
t2 <- infants[infants$ed == "College",]
t2 <- table(t2$smoke,t2$number)
...
over35 <- infants[infants$age >= 35,]
under35 <- infants[infants$age < 35,]

bdiff <- mean(over35$bwt,na.rm=T)-mean(under35$bwt,na.rm=T)
...
coll <- rainbow(length(unique(mtcars$am)))
plot(mtcars$wt,mtcars$disp,col=coll,xlab="weight of the car in lb/1000",
     ylab="the engine displacement in cu")
...
prop.rain <- sapply(rain, function(x) x/365)
...
standardizeVar <- function(m, cols=TRUE){
  if(cols==TRUE){
    standc <- m-colMeans(m)/sd((colSums(m)))
  } else if(cols==FALSE){
    standr <- m-rowMeans(m)/sd((rowSums(m)))
  }
}
